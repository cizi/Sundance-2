...
class SundanceView extends WatchUi.WatchFace {
    ...
    function onLayout(dc) {
        ...
        field4 = [(dc.getWidth() / 13) * 7, ((dc.getHeight() / 4).toNumber() * 3) - 6];		// on F6 [140, 189]
				...
				smallDialCoordsLines = uc.calculateSmallDialLines();
    ...
    function drawDial(dc, today) {
    	...
    	for(var angle = 0; angle < 360; angle+=15) {
      	if ((angle != 0) && (angle != 90) && (angle != 180) && (angle != 270)) {
      		...
      		dc.drawLine(halfScreen, halfScreen, coorsArray[0], coorsArray[1]);
    		}
    	}
		...
		function drawDialNum(coordinatesArray, value, font) {
			var char = null;
			for(var i = 0; i < value.length; i+=1) {
				char = value.substring(i, 1);
    ...
    function drawNrDial(dc) {
    	...
    	for(var nr = 1; nr < 24; nr+=1) {
	      	if ((nr != 6) && (nr != 12) && (nr != 18)) {
						...
						if (hourValue == -1) {	// 23
							drawDialNum(coords, "23", fnt23);
						} else if (hourValue == -2) {	// 22
							drawDialNum(coords, "22", fnt22);
						} else if (hourValue == -3) {	// 21
							drawDialNum(coords, "21", fnt21);
						} else if (hourValue == -4) {	// 20
							drawDialNum(coords, "20", fnt20);
						} else if (hourValue == -5) {	// 19
							drawDialNum(coords, "19", fnt19);
						} else {
							// font = (nr == 1 ? fnt1 : (nr == 2 ? : (nr == 3 ? fnt3 : (nr == 4 ? fnt4 : (nr == 5 ? fnt5 : (nr == 7 ? fnt7 : (nr == 8 ? fnt8 : (nr == 9 ? fnt9 : (nr == 10 ? fnt10 : (nr == 11 ? fnt11 : (nr == 13 ? fnt13 : (nr == 14 ? fnt14 : (nr == 15 ? fnt15 : (nr == 16 ? fnt16 : (nr == 17 ? fnt17 : fnt1)))))))))))))));
							font = "fnt" + hourValue.toString();
							// 15 hodnot fnt
							drawDialNum(coords, hourValue.toString(), *font);
						}
      		}
      	}
    }
